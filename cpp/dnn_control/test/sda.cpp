...
void test_sda() {
  ...
  // training data
  int train_X[train_N][n_ins];
  int train_Y[train_N][n_outs];

  for (int i = 0; i < train_N; ++i) {
    const double val1 = rand() / RAND_MAX / 2.0;
    const double val2 = rand() / RAND_MAX / 2.0;
    train_X[i][0] = val1;
    train_X[i][1] = val2;
    train_X[i][2] = val1 + val2;
    train_Y[i][0] = val1;
    train_Y[i][1] = val2;
    train_Y[i][2] = val1 + val2;
  }
