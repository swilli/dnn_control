...
path_suffix = "fixed_ast_linear"

user_path = os.path.expanduser("~")

training_path = user_path + "/Documents/dnn/data/fixed_ast/training/"
...
for i in range(stacked_autoencoder.n_layers):
    ...
    for epoch in range(pretraining_epochs):
        ...
        print '... ' + time.strftime("%c") + ': pre-training layer %i, epoch %d, cost %f' % (i+1, epoch, mean(c))
    ...
    while epoch < fine_tune_epochs and not done_looping:
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                print '... ' + time.strftime("%c") + ': epoch %i, minibatch %i/%i, validation error %f' % (epoch, minibatch_index + 1,
