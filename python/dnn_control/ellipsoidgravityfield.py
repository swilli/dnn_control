def carlson_elliptic_integral_rd(x, y, z, error_tolerance):
    ...
    low_limit = 5.0 * float_info.min
    max_limit = 5.0 * float_info.max
    ...
    yn_dev = (mu - yn) / mu
    zn_dev = (mu - zn) / mu
    epsilon = max([fabs(val) for val in [xn_dev, yn_dev, zn_dev]])
    ...
    s2 = zn_dev * \
        (coef_2 * ef + zn_dev * (-coef_3 * ec + zn_dev * coef_4 * ea))
...
def ellipsoid_gravity_field(position, semi_axis_a, semi_axis_b, semi_axis_c, density):
    ...
    phi = asin(
        sqrt((semi_axis_a_pow2 - semi_axis_c_pow2) / (maximum_root + semi_axis_a_pow2)))
    k = sqrt((semi_axis_a_pow2 - semi_axis_b_pow2) /
             (semi_axis_a_pow2 - semi_axis_c_pow2))
