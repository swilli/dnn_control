class HiddenLayer:
    ...
    def back_propagate(self, previous_delta_weights):
        from numpy import empty
        ...
        for i in range(self.size_output):
            deltas[i] = 0.0
            for j in range(len(previous_delta_weights[i])):
                deltas[i] += previous_delta_weights[i][j]
            deltas[i] *= self._d_dt_activation_values[i]
